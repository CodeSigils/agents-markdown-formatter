...
Unlike lint-focused tools, this skill focuses on *formatting* with safety guarantees:
...
| `--all` | Process all files in directory recursively |
| `--guard` | Enable structural guard (fence/table drift detection) |
| `--verify` | Run idempotence check (format twice, verify convergence) |
| `--fences` | Validate fence structure with `check-fences.js` |
| `--validate` | Validate table column counts with structural guard |
...
- `test/fixtures/idempotence/` — Files that converge on second formatter pass
- `test/unit/` — Isolated component tests (guard, fences, formatter)
...
- `scripts/` — Guard scripts (check-fences, check-structure, post-write)
- `references/` — Rules documentation and `.oxfmtrc.json` config
